...
const DashBoard = () => {
  ...
  const getData = async () => {
    const stats = await Request.getStatistics(token);
    setStatistics(stats.data);
  };

  const UpdatePNR = async (e) => {
    setShowPNR(e.target.checked);
    const data = {
      showPNR: e.target.checked,
    };
    const response = await Request.UpdatePNR(token, JSON.stringify(data));
    if (response.update) {
